...
delta_t =4.08163265e-7*2;           % Time-step
num_t = 200000;               % Number of time-steps
...
%    if type ==1 || type==2    % Slip/catch bonds
 ...
 time_elapsed/3600      
  ...
  if rem(i,20000)==0 || rem(i,15000)==0
      save(sprintf('Res_ideal_mixed_02_%d.mat',i));
  end
    
  

    
    if delta_t < 8.1632650e-06*1
    delta_t=1.0005*delta_t;
   

    if delta_t> 8.163265e-6*1
        delta_t=8.163265e-6*1;
    end
    end
    ...
    if delta_t < 8.1632650e-07*1.5
    delta_t=1*8.1632650e-07*1.5;
    end
