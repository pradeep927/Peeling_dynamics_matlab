...
% D1              = L0n^2/T0n;                % Diffusionion constant of bonds (m^2/s)
...
% 
% Invariant_main_b.num_el=150;     % Number of elements in the bound region
% Invariant_main_u.num_el=250;    % Number of elements in the unbound region
% discrete_pre_cal2
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 
 X_= [UU; VV; WW];
% 
Mat_param.koff=100*Mat_param.koff;
Mat_param.kon=100*Mat_param.kon;
...
num_t = 100000;               % Number of time-steps
...
%       L(4,1:3)=(Invariant_main_b.Nsh_b_0(1,:)*ddelta_0/Mat_param.x_beta + Invariant_main_b.Nsh_b_0(2,:));
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Remeshing
     if s<s_hist(1)*(Mat_param.aaa/9)
         remeshing_continue2
         Mat_param.aaa=  Mat_param.aaa-1;
         r=[r,i];
     else
     end
...
figure
plot([1:num_t]*delta_t,total_mass(:,1),'ko-')
xlabel('time')
ylabel('total moles')

% Plot the evolution of s
figure(11)
plot([1:num_t]*delta_t,s_hist)
xlabel('time')
ylabel('xi(t)')

% Plot the evolution of the contact angle
figure(21)
plot([1:num_t]*delta_t,angle)
xlabel('time')
ylabel('theta(t)')
...
VV = X(Invariant_main_b.num_cp+1:2*Invariant_main_b.num_cp);
WW = X(2*Invariant_main_b.num_cp+1:end);
[uu,duu,dduu]=interpolate_tot(Invariant_main_b,UU);
[vv,dvv,ddvv]=interpolate_tot(Invariant_main_b,VV);
[ww,dww,ddww]=interpolate_tot(Invariant_main_u,WW);

% Plot mini_model solutions
% Extended domain
plot(Invariant_mini.gaussp,u_g_)
% Rescaled domain
plot(Invariant_main_b.gaussp,u_g)
% Off rate
plot(Invariant_main_b.gaussp,exp(u_g/Mat_param.x_beta))