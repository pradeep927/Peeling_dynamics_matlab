function [M_nl, M_off, A2] = create_special_matrix_rigid(Invariant,v_nl,u_g,du_g,type);
...
for j = 1:num_el
    ...
    for k = 1:gaussno
        ...
        A_loc2=A_loc2+ + B_Matrix'*N_Matrix * 0 * 0* gweight(gp_global);
        
        if type == 1      % Slip bonds
            M_loc3= M_loc3 + N_Matrix'*N_Matrix * gweight(gp_global)*exp(0/Mat_param.x_beta_s);
            
            
        elseif type == 2  % Catch bonds
            f_c = exp(0/Mat_param.x_beta_s)+Mat_param.A*exp(0/Mat_param.x_beta_c);
            g_c = exp(0/Mat_param.x_beta_s)-Mat_param.A*(Mat_param.x_beta_s/Mat_param.x_beta_c)*exp(-0/Mat_param.x_beta_c);
            g_c = g_c/f_c;
            M_loc3= M_loc3 + N_Matrix'*N_Matrix * gweight(gp_global)*f_c;
            A_loc2= A_loc2 + B_Matrix'*N_Matrix *0*g_c*gweight(gp_global);  
        else              % Ideal bonds
