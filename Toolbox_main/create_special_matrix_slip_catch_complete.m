function [M_nl, M_off, A2,A3,M4] = create_special_matrix_slip_catch(Invariant,v_nl,u_g,du_g,type);
...
for j = 1:num_el
    ...
    for k = 1:gaussno
        ...
        M_loc2= M_loc2 + N_Matrix'*N_Matrix *v_nl(gp_global)*exp(-(u_g(gp_global)/Mat_param.x_gamma)^2)*gweight(gp_global);
        A_loc3=A_loc3+ + B_Matrix'*N_Matrix * du_g(gp_global) * u_g(gp_global)* gweight(gp_global);
        M_loc4= M_loc4 + N_Matrix'*N_Matrix * gweight(gp_global)*exp(-(u_g(gp_global)/Mat_param.x_gamma)^2);
    ...
    A3(connect_loc,connect_loc)=  A3(connect_loc,connect_loc) + A_loc3;
    
    M4(connect_loc,connect_loc)=  M4(connect_loc,connect_loc) + M_loc4;
